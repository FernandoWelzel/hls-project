...
model = sf.Sequential([
    # 1st Conv + Pool
    sf.Conv2D(kernel_size=(5, 5), input_shape=(3, 24, 24), output_shape=(64, 24, 24), name="conv1"),
    sf.MaxPooling2D(input_shape=(64, 24, 24), pool_size=(3, 3), strides=(2, 2), name="pool1"),
    
    # 2nd Conv + Pool
    sf.Conv2D(kernel_size=(5, 5), input_shape=(64, 12, 12), output_shape=(32, 12, 12), name="conv2"),
    sf.MaxPooling2D(input_shape=(32, 12, 12), pool_size=(3, 3), strides=(2, 2), name="pool2"),
    
    # 3rd Conv + Pool
    sf.Conv2D(kernel_size=(5, 5), input_shape=(32, 6, 6), output_shape=(20, 6, 6), name="conv3"),
...
axs[0, 1].title.set_text(f"Figure {image_number} normalized channels")
...
fig.suptitle(f'result: {convesion_list[np.argmax(result)]}', fontsize=30)
